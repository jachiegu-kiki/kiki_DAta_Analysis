...
v5.1 變更 (2026-04):
  [語義平移] header.execution_date 為 yesterday（系統執行日）。
    ...
    def _rc_sql(group_sign_expr: str, group_refund_expr: str,
        ...
        return f"""
            WITH sign_agg AS (
                SELECT
                    {gs_bucket} AS bucket,
                    SUM(CASE WHEN sign_date=:today THEN gross_sign ELSE 0 END)/10000.0 AS d_gs,
            ...
            refund_agg AS (
                SELECT
                    {rf_bucket} AS bucket,
                    SUM(CASE WHEN refund_date=:today THEN gross_refund ELSE 0 END)/10000.0 AS d_rf,
