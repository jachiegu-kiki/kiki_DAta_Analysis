...
FILES = {
    ...
    "eurasia_signing": (PREDATA_DIR, "欧亚事业部签约表.xlsx",          "收入人次",       0),
...
# ───────────────────────────────────────────────────────────────
# 事实表「受管数据分层」——全 ETL 唯一真相源（single source of truth）
# ───────────────────────────────────────────────────────────────
# write_signing / write_refund 每次同步会【无条件】清空这些 source 分层后全量重灌，
# 以此保证「表中受管分层 == 本次抽取结果」的幂等不变量，杜绝孤儿分层残留（double 根因）。
#
# ⚠ 字面值必须与 DB CHECK 约束完全一致（简体：历史/月更/日更，非繁体「歷史」）。
#    与 database/01_schema.sql 的 CHECK (source_system IN ('日更','月更','历史')) 对齐，
#    与 signing.py / refund.py 各 mod_* 写入的 source 值对齐。
#    若将来新增一个由本 ETL 全量管理的 source，必须且只需在此处登记一行。
MANAGED_SOURCES = ("历史", "月更", "日更")
